...
def auto_add_roiextractor_methods():
    ...
    for method_name in dir(ImagingExtractor):
        if not method_name.startswith("_") and callable(getattr(ImagingExtractor, method_name)):
            if hasattr(BaseROIExtractorImagingSegment, method_name):
                continue
            
            def make_wrapper(method_name):
                def wrapper(self, *args, **kwargs):
                    return getattr(self.roiextractor_extractor, method_name)(*args, **kwargs)
                wrapper.__name__ = method_name
                return wrapper
            
            setattr(BaseROIExtractorImagingSegment, method_name, make_wrapper(method_name))


def read_imaging(file_path: str, imaging_name: str | None = None, **kwargs) -> BaseROIExtractorImaging:
